...
- **Criação manual**: `criarAgendamentoServico` (`src/actions/agendamentos.ts`) — valida funcionamento (`validarFuncionamento`), profissional habilitado, conflito de profissional (`buscarConflito`, já agnóstico de modalidade) e resolve sala; cria com `status: "AGENDADO"`, sem `planoAtribuicaoId`/`gradeRecorrenteId`. UI: `AgendamentoServicoDialog` (`src/components/pacientes/agendamento-servico-dialog.tsx`) — form simples (não é wizard de disponibilidade como o de Plano, já que não há grade fixa nem orçamento), profissional filtrado dinamicamente por serviço via `getProfissionaisServico`.
