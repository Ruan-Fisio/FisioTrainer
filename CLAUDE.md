...
- **Plano mensal nunca é parcelado** (parcela única) e **trimestral parcela em até 3x** (só na forma `ATE_3X_CARTAO`; trimestral à vista = 1 parcela). Regra central em `maxParcelasPlano(periodicidade, formaPagamento, permiteParcelamentoEstendido?)` e `formaEfetiva(periodicidade, formaPagamento, permiteParcelamentoEstendido?)` (`src/lib/planos.ts`) — mensal força `A_VISTA`, exceto com parcelamento estendido. A server action normaliza a forma com `formaEfetiva` antes de gravar; o teto exato por plano é checado em `createPlanoAtribuicao`/`updatePlanoAtribuicao` (`src/actions/plano-atribuicoes.ts`) depois de buscar o `Plano` — o `planoAtribuicaoSchema` só garante um teto genérico (6, `MAX_PARCELAS_PLANO_HARD_CAP`), pois não tem acesso ao registro `Plano` (só ao `planoId`).
- **Parcelamento estendido**: checkbox "Permite parcelamento estendido" no cadastro do Plano (`Plano.permiteParcelamentoEstendido`, boolean) libera trimestral até 6x e mensal até 2x, sem novo campo de preço — as parcelas extras dividem o mesmo valor total já cadastrado (`valorAte3xTrimestral`/`valorAVistaMensal`). **Só aparece e só é aceito no plano híbrido** (`tipos` contém `FISIOTERAPIA` **e** `EDUCACAO_FISICA` ao mesmo tempo) — em `plano-form.tsx` o checkbox fica escondido e é desligado automaticamente (`useEffect`) se o usuário desmarcar um dos dois tipos; no servidor, `planoSchema` rejeita `permiteParcelamentoEstendido: true` fora do plano híbrido via `.refine`.
...
- No `plano-atribuicao-form.tsx` a seção "Forma de pagamento" aparece quando a periodicidade é `TRIMESTRAL`, ou em `MENSAL` se o plano selecionado tem `permiteParcelamentoEstendido`; senão mostra só o aviso de parcela única. O label "Até Nx no cartão" é sempre calculado por `maxParcelasPlano` (nunca hardcoded "3x"), já que o teto varia por plano.
